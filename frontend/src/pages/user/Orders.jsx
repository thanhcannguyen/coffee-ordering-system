            ...
            <div style={s.container}>
                ...
                {orders.length === 0 ? (
                    <div style={s.emptyBox}>
                        ...
                        <button style={s.btnPrimary} onClick={() => navigate('/menu')}>Đặt hàng ngay</button>
                    </div>
                ) : (
                    <div style={s.list}>
                        {orders.map(order => {
                                ...
                                <div key={order._id} style={s.card} onClick={() => navigate(`/orders/${order._id}`)}>
                                    ...
                                    <div style={s.itemsPreview}>
                                        {order.items.slice(0, 3).map((item, idx) => (
                                            <span key={idx} style={s.itemTag}>{item.name} x{item.quantity}</span>
                                        ))}
                                        {order.items.length > 3 && <span style={s.itemTag}>+{order.items.length - 3} món khác</span>}
                                    </div>
                                    <div style={s.cardFooter}>
                                        <span style={s.footerMeta}>{order.items.reduce((s, i) => s + i.quantity, 0)} món · {order.paymentMethod}</span>
                                        <span style={s.totalAmount}>{order.totalAmount.toLocaleString('vi-VN')}đ</span>
                                    </div>
                                </div>
                            )
                        })}
...
const s = {
    ...
    container: { maxWidth: 760, margin: '0 auto', padding: '0 16px' },
    ...
    card: { background: '#fff', borderRadius: 12, border: '1px solid #e8ddd5', padding: '16px', cursor: 'pointer', transition: 'box-shadow 0.15s' },
    cardHeader: { display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10, flexWrap: 'wrap', gap: 8 },
    orderId: { fontSize: 14, fontWeight: 600, color: '#1a0f0a', marginRight: 8 },
    orderDate: { fontSize: 12, color: '#8b7355' },
    badge: { fontSize: 12, fontWeight: 500, padding: '4px 12px', borderRadius: 20, flexShrink: 0 },
    itemsPreview: { display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 12 },
    itemTag: { fontSize: 12, background: '#f5f0eb', color: '#6f4e37', padding: '3px 10px', borderRadius: 20 },
    cardFooter: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: '1px solid #f5f0eb', paddingTop: 10, flexWrap: 'wrap', gap: 6 },
